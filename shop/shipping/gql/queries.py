...
class GeoQueries(graphene.ObjectType):
    ...
    states_by_country = graphene.List(
        gql_types.StatesType,
        country=graphene.String())
    cities_by_country_state = graphene.List(
        gql_types.CitiesType,
        country=graphene.String(),
    ...
    def resolve_states_by_country(root, info, country):
        return (gql_types.StatesType(name=state)
                for state in CountriesApi.get_states_by_country(country))

    def resolve_cities_by_country_state(root, info, country, state):
        return (
            gql_types.CitiesType(
                name=city) for city in CountriesApi.get_cities_by_country_state(
                country,
                state))
